    ...
    if (await isOverFlow(serverInfo, accountInfo)) {
      ...
      if (ssr_exists) {
        ...
        if (accountInfo.type > 1 && accountInfo.type < 6 && accountInfo.server && JSON.parse(accountInfo.server).indexOf(serverInfo.id) == 0) {
          ...
          emailPlugin.sendMail(user.email, '封停提醒', `您的账号[ ${accountInfo.port} ]当期预置流量已耗尽，账号已暂停使用，请购买流量包或续费高级套餐恢复！(续费当前套餐不会马上恢复，下个周期才会恢复)不限量套餐请联系管理员或回复本邮件重置，谢谢合作！(一般会在24小时内处理，节假日顺延)`);
        }
      }
      return;
    }
