...
const v2_quan = (account, server, title) => {
  ...
  return 'vmess://' + urlsafeBase64(`${server.name}=vmess,${server.host},${server.v2rayPort},${server.v2rayMethod},"${account.uuid}",group=${title},over-tls=${!!server.v2rayTLS},certificate=${server.v2rayTLS || 0},obfs=${server.v2rayNet || 'tcp'},obfs-path="${server.v2rayPath || ''}",obfs-header="Host: ${server.v2rayHost}[Rr][Nn]User-Agent: Test Agent"`);
}
// v2 QuanX
...
const v2_quanx = (account, server) => {
  ...
  return `vmess=${server.host}:${server.v2rayPort},method=${server.v2rayMethod},password=${account.uuid},obfs=${server.v2rayNet}${server.v2rayNet == 'wss' ? ',obfs-host=' + server.v2rayHost + ',obfs-uri=' + server.v2rayPath : ''},fast-open=false,udp-relay=false,tag=${server.name}`;
}
//SS 默认链接
